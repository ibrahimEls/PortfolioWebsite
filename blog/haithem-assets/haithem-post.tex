...
In addition to the LLaM, \haithem{} contains a decision tree construction that uses experimental observables that are yet to be measured as ways to differentiate viable regions the LLaM finds. We compute many observables analytically, and construct the tree based on bins of those observables. If the decision tree at a leaf node contains more than one set of viable regions, we have found more than one description of reality that shares exactly the same \textit{signature} (if all future experiments we consider were built). In this case, we employ LLM-agents to continue the splitting, asking them to search the literature or propose novel observables that can split the set of parameter space regions over a set of Lagrangians that share the same signature.
...
Breaking these degeneracies is the job of LLM-agents, which can reason at scale (needed when we have hundreds of these leaves). We hand these leaves to an LLM-agent in two stages. First, we ask for a literature search, where the agent must find an existing or
planned measurement outside what we compute that separates the regions. It must then verify every reference it cites on arXiv, and say whether its proposal succeeded in breaking the degeneracy.
