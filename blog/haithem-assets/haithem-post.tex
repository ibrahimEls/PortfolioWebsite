...
We let the search for region in parameter space not excluded by experiment be akin to a single-player game of battleship. The board is defined by a Lagrangian, some set experimental
...
The Large Lagrangian Model that is trained to learn how to play this game is created with an autoregressive in parameter space transformer whose tokens are Lagrangians and their phenomenology.  Each episode, it is given one Lagrangian and a budget of between 5 and 50 turns. 
On every turn it proposes four distributions over that theory's parameter space and some the set shots for that turn is drawn from them. The outcome of the current turn is given to the LLaM for it to use for the next turn(s).

The model takes as input three things. The tokenized Lagrangian, the viability configuration (which experimental cuts are active), what ranges the agent can search in, and how many turns it has left. It also takes an encoding of the search so far, where 64 learned tokens
cross-attend over every probe taken across the game. This set up lets each proposal depend on what the previous turns revealed.

Each of the four heads outputs a Beta distribution over every parameter. Independent Betas cannot express correlations between parameters, so
...
Below are benchmarks of the LLaM against a Differential Evolution (DE) baseline. The LLaM finds more viable points than DE, esspeicaly with small number of probes. The LLaM also does well with increasing Lagrangian parameter dimension, while DE collapses, as is expected with the curse of dimensionality.
...
\pullquote{The Large Lagrangian Model finds what theories to consider, and the LLM-agent does the phenomenology there, both working at scale}

We take all viable points the LLaM found for a Lagrangian, and
grow a tree with them, where each node is one observable, and each leaf a distinct combination of signatures contains all viable points that share the same signatures. After growing the tree, we find leaves holding more than one region. We consider these degenerate in that is nothing we currently compute tells the theories in the leaf apart. 

Breaking these degeneracies is the job of the LLM-agents, which can reason at scale (needed when we have hundreds of these leaves) . We hand these leaves to an LLM-agent in two stages. First, we ask for a literature search, where the agent must find an existing or
planned measurement outside what we compute that separates the regions. It must then verify every reference it cites on arXiv, and say whether its proposal succeeded to break the degeneracy.

If nothing published splits the leaf, or splits only part of it, the agent moves to the second stage, which is to propose a new observable. It does so and rates how feasible the new observable is.
