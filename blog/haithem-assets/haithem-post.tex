...
\pullquote*[What are unexplored, realistic and relevant \textbf{observables} that distinguish descriptions?]{What is the best \textbf{description} of
reality?}

Large Language Models (LLMs) may be able to propose useful observables that are efficient at disambiguating theories, but they currently struggle with open-ended questions and may be biased toward previously explored observables inspired by Lagrangians or regions of Lagrangian parameter space with high density in the literature

We introduce \haithem{} (\textbf{H}idden-sector \textbf{AI} for \textbf{T}estable \textbf{H}ypothesis \textbf{E}nu\textbf{M}eration), a framework designed to make observable generation tractable, backronymed after a pioneer of the experimental method, Ibn Al-Haytham (965-1040 CE), whose work cemented controlled experiment as the arbiter of competing theory \cite{Sabra:1989optics,Ibn-al-haythem-reflect}. \haithem{} turns the task of signature generation into a competition between candidate signatures' ability and practicality for separating two or more descriptions of reality that satisfy current experimental constraints.

\interactive{param3d}{Watch a trained Large Lagrangian Model play the
battleship-like search over parameter space.}

To do so, \haithem{} is first composed of what we call a Large Lagrangian Model (LLaM), an autoregressive transformer pretrained from scratch on over 1 Billion Output Tokens from over $10,000$ specially tokenized Lagrangians, and fine-tuned in a live environment with reinforcement learning (RL). The LLaM is trained to play a battleship-style game (with limited turns and limited probes per turn) over the parameter space of a theory. Its goal is to find regions in that parameter space that can be the correct description of reality (that is, no experiment we check rules them out).
...
With a set of non-excluded regions found by the LLaM, \haithem{} constructs a decision tree that generates the competition between candidate signatures. The decision tree is built by using analytically computed experimental observables as ways to differentiate viable regions the LLaM finds. We compute many observables and construct the tree based on bins of those observables. If the decision tree at a leaf node contains more than one set of viable regions, we have found more than one description of reality that shares exactly the same \textit{signature} given the experiments we consider. In this case, we turn the degeneracy into a question and employ an LLM-agent to continue the splitting, asking it to search the literature or propose observables that can split the leaf.

This approach stands in contrast to open-endedly querying an LLM about what observables may test a class of models. In that case, the language model will tend to give answers as vague as the question, and likely biased toward signatures well established in the literature. On the other hand, a tree construction turns breaking degenerate leaves (leaves with more than one parameter-space region that share every experimental signature we compute) into a particular task with specific Lagrangians and specific parameter values (masses, couplings, etc.). The LLM must give sharp binary verdicts as to what and how degenerate regions differ in its proposal. Additionally, it is forced to think beyond observables already analytically computed earlier in the tree and is allowed null answers (no signature found). We further find that the decision tree formulation leads to a natural grouping of Lagrangian-viable regions into experimentally distinguishable classes. 
...
\figplace[wide]{paleo-detector}{The agent's novel alternative. A gypsum
sample buried for about a billion years records every nuclear recoil in its
crystal lattice; the ratio of the longest calcium and oxygen tracks fixes the
dark matter mass while the dependence on the galactic escape velocity cancels.}
