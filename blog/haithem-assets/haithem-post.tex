...
%    \alphazero
%    \papercard            -> the "The paper" block above the references: the
%                            paper's title, authors and abstract, which are
%                            also the page's search metadata
...
% the paper this post accompanies. Printed on the page under "The paper",
% just above the references, and repeated in the page's metadata so the
% paper's title and abstract find their way here
...
\newcommand{\papercard}{%
  \section*{The paper}
  \noindent{\large\thepapertitle}\par\medskip
  \noindent\thepaperauthors\par\smallskip
  \begin{enumerate}\itemsep0pt \thepaperaffil \end{enumerate}
  \noindent\textbf{Abstract.} \thepaperabstract\par}
...
\papertitle{Searching for BSM Experimental Signatures with Large Lagrangian
Models}
\paperauthors{Ibrahim Elsharkawy$^{1,2}$, Victoria Knapp-Perez$^{3,4}$,
Wahid Bhimji$^{2}$, Aishik Ghosh$^{5,6}$}
\paperaffil{%
  ...
  \item Department of Physics and Astronomy, University of California, Irvine,
        CA 92697, USA
  \item Halluminate, San Francisco, California, USA, 94107
  \item Georgia Institute of Technology, Atlanta, GA 30332, USA
  \item Physics Division, Lawrence Berkeley National Laboratory, Berkeley,
        CA 94720, USA}
\paperabstract{The search for Beyond-Standard-Model (BSM) physics is generally
limited not by the supply of theory descriptions but by the lack of
discriminating experimental observations. A case in point is dark matter, where
the overwhelming gravitational evidence only goes so far in distinguishing
between models within a vast theory space. In such cases, exploring the space of
testable model signatures may help identify overlooked experimental observables
and indicate which future experiments are useful for breaking degeneracies
within a class of models. A challenge is designing a search that explores models
and their signatures outside what is found in the literature. We introduce
\haithem{}, a framework built to do just that, composed of two stages. The first
is a reinforcement-learning agent that learns to find portions of a theory's
high-dimensional parameter space that are not excluded by playing a
Battleship-style ``game''. The agent is built as a Large Lagrangian Model
(LLaM), an autoregressive transformer that reads a tokenized Lagrangian, is
pretrained at scale (here on $\sim$1 billion tokens from $\sim$10,000
Lagrangians), and is fine-tuned in a live environment. The second is a
decision-tree construction that first separates LLaM-found regions with
analytically computed observables, then passes remaining degenerate regions to a
set of LLM agents that compete in producing realistic signatures. For this
proof-of-concept, we focus on the space of thermal WIMPs. Compared to a
genetic-algorithm-based baseline, the LLaM-search discovers more viable regions
and greater physical diversity within them. As an example of how such a tool
could be useful, we search for signatures over a set of single-dark-multiplet
models. Already in this well-studied space, \haithem{} proposes combinations of
observables not previously studied, for example, the application of a
halo-independent kinematic ratio to paleo-detectors.}

\indexpretitle{Blog}
\indextitle{Research Thoughts}
\postexcerpt{The search for Beyond-Standard-Model (BSM) physics is limited not by the supply of theory descriptions but by the lack of discriminating experimental observations. In such cases, exploring the space of testable model \textit{signatures} may help identify overlooked experimental observables. We introduce \haithem{}, a framework built to do just that, composed of Large Lagrangian Models and LLM-agents.}
\postmore{Read post $\rightarrow$}
\postthumbalt{Viable points found by the search in a Lagrangian's parameter
space}
...
\videoplace{tree-growth}{The global signature tree as it grows.}
...
One motivating example is a ``Novel Alternative" shown in two nodes in the global tree, titled \textit{``Paleo-detector Ca/O recoil-spectrum ratio"}. In both cases, the agent was in need of a precise measurement of the dark matter mass $m_\chi$.  The agent recognizes that \cite{Drees:2008bv}'s description of the expected maximum kinetic energy a dark matter particle deposits in a direct detection detector is a good probe for $m_\chi$ and is given by,
\begin{equation}
E_{\rm max} \sim v^2_{\rm esc} f(m_\chi)
\end{equation}
where $v_{\rm esc}$ is the galactic escape velocity of the dark matter particles, $ f(m_\chi)$ is some known function of the dark matter mass. Thus, a statistically robust measurement of $E_{\rm max}$ may be a precise probe of the dark matter mass if $v_{\rm esc}$ is known.

The challenge is that the exposure needed (the amount of time a detector is collecting data) for measuring such an observable is very large, requiring many dark matter detections near $v_{\rm esc}$.

As a solution, the agent reaches for paleo detectors, minerals found deep underground, protected from cosmic rays and high temperatures for billions of years. Disturbances of the crystal lattice of these detectors contain permanent records of potential dark matter interactions \cite{Baum:2018tfw,Baum:2023cct,Baum:2021jak,Edwards:2018hcf,Drukier:2018pdy}.

However, by the nature of their long exposure taking place throughout our galaxy's history, paleo detectors introduce much more uncertainty in the galactic escape velocity $v^2_{\rm esc}$. As a solution, the agent proposes the use of the ratio of two direct detection targets,
\begin{equation}
    \frac{E_{\rm max}^{N_1}}{E_{\rm max}^{N_2}} \equiv f(m_\chi)
\end{equation}
which removes $v^2_{\rm esc}$ dependency entirely.

It further reasons that identifying a single mineral containing two sufficiently sensitive target nuclei would be ideal for controlling error. For example, neutrons from radioactive decays of surrounding elements. It identifies calcium and oxygen in gypsum as such a pair, inspired by recent work in \cite{Theodosopoulos:2026eym} (though they do not consider this observable).
...
\bibitem{Drees:2008bv}
M.~Drees and C.-L.~Shan, \emph{Model-Independent Determination of the WIMP Mass
from Direct Dark Matter Detection Data}, JCAP \textbf{06} (2008) 012,
\href{https://arxiv.org/abs/0803.4477}{arXiv:0803.4477}.

\bibitem{Baum:2018tfw}
S.~Baum, A.~K.~Drukier, K.~Freese, M.~G\'orski and P.~Stengel,
\emph{Searching for Dark Matter with Paleo-Detectors}, Phys. Lett. B
\textbf{803} (2020) 135325,
\href{https://arxiv.org/abs/1806.05991}{arXiv:1806.05991}.

\bibitem{Baum:2023cct}
S.~Baum \emph{et al.}, \emph{Mineral detection of neutrinos and dark matter. A
whitepaper}, Phys. Dark Univ. \textbf{41} (2023) 101245,
\href{https://arxiv.org/abs/2301.07118}{arXiv:2301.07118}.

\bibitem{Baum:2021jak}
S.~Baum, T.~D.~P.~Edwards, K.~Freese and P.~Stengel, \emph{New Projections for
Dark Matter Searches with Paleo-Detectors}, Instruments \textbf{5} (2021) 21,
\href{https://arxiv.org/abs/2106.06559}{arXiv:2106.06559}.

\bibitem{Edwards:2018hcf}
T.~D.~P.~Edwards, B.~J.~Kavanagh, C.~Weniger, S.~Baum, A.~K.~Drukier,
K.~Freese, M.~G\'orski and P.~Stengel, \emph{Digging for dark matter: Spectral
analysis and discovery potential of paleo-detectors}, Phys. Rev. D \textbf{99}
(2019) 043541, \href{https://arxiv.org/abs/1811.10549}{arXiv:1811.10549}.

\bibitem{Drukier:2018pdy}
A.~K.~Drukier, S.~Baum, K.~Freese, M.~G\'orski and P.~Stengel,
\emph{Paleo-detectors: Searching for Dark Matter with Ancient Minerals},
Phys. Rev. D \textbf{99} (2019) 043014,
\href{https://arxiv.org/abs/1811.06844}{arXiv:1811.06844}.

\bibitem{Theodosopoulos:2026eym}
D.~P.~Theodosopoulos, K.~Freese, C.~Kelso and P.~Stengel, \emph{Reconstructing
Dark Matter Mass and Discriminating Standard and Non-Standard WIMP-Nucleus
Interactions with Paleo-Detectors} (2026),
\href{https://arxiv.org/abs/2608.10105}{arXiv:2608.10105}.

\end{thebibliography}

% ----------------------------------------------------------- listing page --
% Not part of the post. This is the text on blog.html: the heading of the
% listing page and the card that links here.
\indexcard
